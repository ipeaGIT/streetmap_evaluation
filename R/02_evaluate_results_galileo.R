...
## Galileo results
galileo_geocoded_df <- read_delim("../../data/geocode/streetmap_eval/Galileo/cnefe_galileo_output.csv",
                                  delim = ";",
...
output_csv <- "../../data/geocode/streetmap_eval/validated_sample_galileo.csv"
...
validate_geocoding <- function(muni) {
  ...
  galileo_sf <- galileo_geocoded_sf %>%
    st_set_geometry(NULL) %>% setDT()

  sample_df[galileo_sf, on = .(cnefe_id),
