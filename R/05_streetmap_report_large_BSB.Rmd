...
```{r cars}
validated_sample_df <- 
  read_csv("../../../data/geocode/streetmap_eval/validated_large_sample_completo.csv") %>%
...
test_cutoffs <- function(cutoff) {
  ...
  if (!("WRONG_HIGH" %in% names(validated_scores))) {
    validated_scores$WRONG_HIGH <- 0
  }
  if (!("CORRECT_LOW" %in% names(validated_scores))) {
    validated_scores$CORRECT_LOW <- 0
  }
  if (!("CORRECT_HIGH" %in% names(validated_scores))) {
    validated_scores$CORRECT_HIGH <- 0
  }
